...
func (p *Prompt) SetOptions(opts ...Option) {
...
func (p *Prompt) feed(b []byte) (shouldExit bool, exec *Exec) {
	...
	if !p.handleCompletionKeyBinding(key, p.completion.Completing()) {
		return
	}
	...
	case ControlC:
		...
		p.history.Clear()
	case Up, ControlP:
		if !p.completion.Completing() { // Don't use p.completion.Completing() because it takes double operation when switch to selected=-1.
...
func (p *Prompt) handleCompletionKeyBinding(key Key, completing bool) bool {
	continues := true
	switch key {
	case Down:
	...
	case Enter, ControlM, ControlJ:
		// hack by icy: when press ENTRY key:
		// if there completion list has already select (by DOWN key), but not set into text, just update it
		if p.completion.selected >= 0 {
		...
		} else if len(p.completion.tmp) > 0 && p.completion.selected == -1 {
			// if there completion list has some suggestion, select the first one automatically
			p.completion.Next()
			if s, ok := p.completion.GetSelectedSuggestion(); ok {
				w := p.buf.Document().GetWordBeforeCursorUntilSeparator(p.completion.wordSeparator)
				if w == s.Text {
...
func (p *Prompt) handleKeyBinding(key Key) bool {
	...
	for i := range p.keyBindings {
		...
		if kb.Key == key {
			kb.Fn(p.buf)
			// meat user input sth in binding functions
			p.renderer.BreakLine(p.buf)
			p.buf = NewBuffer()
		}
	}
